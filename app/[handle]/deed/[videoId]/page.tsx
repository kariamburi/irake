...
export default function PlayerByHandlePage() {
    ...
    // Wire HLS to main video element
    useHls(videoRef, item?.mediaUrl || undefined);

    // reset ready when item changes
    useEffect(() => {
        setReady(false);
    }, [item?.id]);

    // prev / next ids based on siblings
    const prevId = useMemo(
        () => (idx > 0 ? siblings[idx - 1]?.id : undefined),
        [idx, siblings]
    );
    const nextId = useMemo(
        () =>
            idx >= 0 && idx < siblings.length - 1 ? siblings[idx + 1]?.id : undefined,
        [idx, siblings]
    );

    const goPrev = () => {
        if (prevId)
            router.push(`/${encodeURIComponent(handleWithAt)}/deed/${prevId}`);
    };
    const goNext = () => {
        if (nextId)
            router.push(`/${encodeURIComponent(handleWithAt)}/deed/${nextId}`);
    };

    // Prefetch prev/next routes to smooth navigation
    useEffect(() => {
        if (!("prefetch" in router)) return;
        if (prevId)

            (router as any).prefetch(
                `/${encodeURIComponent(handleWithAt)}/deed/${prevId}`
            );
        if (nextId)

            (router as any).prefetch(
                `/${encodeURIComponent(handleWithAt)}/deed/${nextId}`
            );
    }, [prevId, nextId, handleWithAt, router]);
    ...
    return (
        <div className="fixed inset-0 bg-black text-white">
                                ...
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    >
                                        ...
                                        <div className="text-white/70 text-[11px]">
                                            {/* follower count handled by hook */}
                                        </div>
                                    </div>
                ...
                <aside className="hidden lg:flex flex-col overflow-y-hidden border-l border-gray-200 bg-white text-gray-900">
                    ...
                    <div className="border-b border-gray-200 p-4">
                        ...
                        <div className="flex items-center gap-4 text-sm">
                            <span>
                                Views: <b>{nfmt(item.stats?.views)}</b>
                            </span>
                            <span>
                                Likes: <b>{nfmt(item.stats?.likes)}</b>
                            </span>
                            <span>
                                Comments: <b>{nfmt(item.stats?.comments)}</b>
                            </span>
                            <span>
                                Shares: <b>{nfmt(item.stats?.shares)}</b>
                            </span>
                        </div>
                    </div>
            ...
            {/* 🔥 Adjacent video preloading for smoother next/prev */}
            <AdjacentPreloadWeb siblings={siblings} activeIndex={idx} />
        </div>
    );
}
