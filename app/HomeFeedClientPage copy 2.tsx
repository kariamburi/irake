...
import { PlayerItem } from "@/lib/fire-queries";
...
import { Deed, FeedCursor, FeedTabKey, fetchChannelPage } from "./deeds/data/deedsFeedWeb";
...
function createInitialFeeds(): Record<FeedTabKey, FeedState> {
    return {
        forYou: { ...EMPTY_STATE },
        following: { ...EMPTY_STATE },
        nearby: { ...EMPTY_STATE },
    };
}
function deedToPlayerItem(deed: Deed): PlayerItem {
    return {
        id: deed.id,
        authorId: deed.authorId,
        authorUsername: deed.authorUsername ?? "",
        authorPhotoURL: deed.authorPhotoURL ?? "",
        text: deed.text ?? "",
        createdAt: deed.createdAt ?? null,
        visibility: deed.visibility ?? "public",
        mediaType: deed.type === "video" ? "video" : "photo",
        mediaUrl: deed.mediaUrl ?? null,
        posterUrl: deed.posterUrl ?? null,
        muxPlaybackId: deed.muxPlaybackId ?? null,
        stats: {
            views: Number(deed.stats?.views ?? 0),
            likes: Number(deed.stats?.likes ?? 0),
            comments: Number(deed.stats?.comments ?? 0),
            shares: Number(deed.stats?.shares ?? 0),
            bookmarks: Number(deed.stats?.bookmarks ?? deed.stats?.saves ?? 0),
            saves: Number(deed.stats?.saves ?? deed.stats?.bookmarks ?? 0),
        },
        music: deed.music,
        authorBadge: deed.authorBadge,
        media: deed.media ?? [],
        type: deed.type,
        orientation: deed.orientation ?? null,
        aspectRatio: deed.aspectRatio ?? null,
        videoWidth: deed.videoWidth ?? null,
        videoHeight: deed.videoHeight ?? null,
    } as PlayerItem;
}
function useDeedsFeed({
    uid,
    archivePage,
    isArchiveMode,
}: {
    ...
    const [feeds, setFeeds] = useState<Record<FeedTabKey, FeedState>>(createInitialFeeds());
    ...
    const loadInitialChannel = useCallback(
        async (tab: FeedTabKey) => {
            ...
            try {
                ...
                const deedItems = Array.isArray(page?.items) ? page.items : [];
                ...
                console.error(`${tab} initial load error:`, error);
    ...
    const loadMoreChannel = useCallback(
        async (tab: FeedTabKey) => {
            const current = feedsRef.current[tab];
            ...
            try {
                ...
                console.error(`${tab} loadMore error:`, error);
    ...
    useEffect(() => {
        ...
        if (!current.initialized && !current.loading) {
            void loadInitialChannel(activeTab);
            return;
        }

        setItems(current.items.map(deedToPlayerItem));
        setHasNextPage(current.hasMore);
        setLoading(current.loading);
    }, [activeTab, feeds, isArchiveMode, loadInitialChannel, uid]);

    const loadMore = useCallback(async () => {
        if (isArchiveMode) return;

        const tab = activeTab;
        await loadMoreChannel(tab);

        const next = feedsRef.current[tab];
        setItems(next.items.map(deedToPlayerItem));
        setHasNextPage(next.hasMore);
    }, [activeTab, isArchiveMode, loadMoreChannel]);
    ...
    const reload = useCallback(async () => {
        ...
        try {
            ...
            setItems(deedItems.map(deedToPlayerItem));
    ...
    }, [activeTab, isArchiveMode, setFeedState, uid]);
...
function FeedShell({
    ...
    const isArchiveMode = archiveMode === "deeds";
    const archivePageNumber =
        isArchiveMode && typeof forcedArchivePage === "number" && forcedArchivePage > 0
            ? forcedArchivePage
            : 1;
    ...
    const {
        ...
        uid,
        archivePage: archivePageNumber,
        isArchiveMode,
    });

    const tab = activeTab as TabKey;
    const setTab = (next: TabKey) => {
        if (!uid && next !== "forYou") return;
        setActiveTab(next);
    };
    ...
    }, [isArchiveMode, index, items.length, loading, loadMore]);
        ...
        <MuteProvider>
            {isMobile ? (
                <MobileShell
                    ...
                    preloadMode={undefined}
                    index={index}
                    hlsMaxHeight={hlsMaxHeight}
                    goUpload={goUpload}
                    changeTab={changeTab}
                    archiveMode={archiveMode}
                    archivePageNumber={archivePageNumber}
                ...
                >
                    ...
                    {isArchiveMode && (
                        <div className="w-full max-w-5xl mx-auto px-4 pb-10">
                            <div className="flex items-center justify-between border-t pt-6" style={{ borderColor: EKARI.hair }}>
                                <div>
                                    {archivePageNumber > 1 ? (
                                        <a
                                ...
                                <div className="text-sm font-semibold" style={{ color: EKARI.subtext }}>
...
function MobileShell(props: any) {
    ...
    return (
        <div className="fixed inset-0">
            ...
            {archiveMode === "deeds" && items.length > 0 && (
...
export default function HomeFeedClientPage({
    ...
    const isDeedsRoute =
        pathname === "/deeds" ||
        pathname.startsWith("/deeds/page/") ||
        isArchiveMode;
    ...
    useEffect(() => {
        ...
        if (isArchiveMode && archivePageNumber > 1) {
            router.prefetch(`/deeds/page/${archivePageNumber - 1}`);
        }
        router.prefetch(`/deeds/page/${archivePageNumber + 1}`);
    }, [router, isArchiveMode, archivePageNumber]);
    ...
    }, [authLoading, user?.uid, router, splashSeen, isDeedsRoute]);

    if (!isDeedsRoute && phase === "splash" && !splashSeen) {
        return (
    ...
    return (
        <FeedShell
            forcedArchivePage={forcedArchivePage}
            archiveMode={archiveMode}
        />
    );
}
