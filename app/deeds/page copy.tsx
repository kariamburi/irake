...
// change this path if your RightRail lives elsewhere
...
import { DeedsScrollerWeb } from "./components/DeedsScrollerWeb";
...
function useDeedBox(params: {
  ...
  useEffect(() => {
    const recalc = () => {
      ...
      const railW = railOpen && vw >= DESKTOP_BREAKPOINT ? DESKTOP_RAIL_W : 0;
      const leftW = vw >= DESKTOP_BREAKPOINT ? DESKTOP_LEFT_W : 0;
...
function DesktopTopBar(props: {
  uid?: string | null;
  profile?: { photoURL?: string | null; handle?: string | null } | null;
...
export function FeedShell({
  ...
  const TAB_BAR_H = isDesktop ? 84 : 56;
  ...
  const emptyText = useMemo(() => {
    ...
    if (feed.activeTab === "nearby") {
      return uid ? "No nearby deeds yet." : "Sign in to see nearby deeds.";
    }
  ...
  const renderScroller = (desktop: boolean) => {
    if (currentFeed.loading && !currentFeed.initialized) {
      return (
        <div
          className={desktop ? "flex items-center justify-center text-black/60" : "flex items-center justify-center text-white/80"}
          style={{ height: cardH }}
        >
          Loading deeds...
        </div>
    ...
    if (!currentFeed.loading && currentFeed.items.length === 0) {
      return (
        <div
          className={desktop ? "flex items-center justify-center px-6 text-center text-black/60" : "flex items-center justify-center px-6 text-center text-white/80"}
          style={{ height: cardH }}
        >
          {emptyText}
        </div>
