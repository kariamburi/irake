...
export function DeedsScrollerWeb({
    ...
    const safeInitialIndex = Math.max(
        0,
        Math.min(Math.max(0, items.length - 1), initialIndex)
    );
    ...
    useEffect(() => {
        ...
        if (!root || !items.length || cardH <= 0) return;
        ...
        requestAnimationFrame(() => {
            ...
            if (item) onActiveItemChange?.(item, clamped);
        });
    }, [initialIndex, cardH, items, onActiveItemChange, rootRef]);

    useEffect(() => {
        const node = rootRef.current;
        if (!node || !items.length || cardH <= 0) return;
        ...
        const updateActive = () => {
            ...
            if (activeItem) onActiveItemChange?.(activeItem, nextIndex);
        };
    ...
    useEffect(() => {
        ...
        if (activeItem) onActiveItemChange?.(activeItem, next);
