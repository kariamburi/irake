...
type AnalyticsSummary = {
    ...
    totalPlayTimeSec?: number;          // lifetime seconds watched
    avgWatchSec?: number;               // average watch time (sec)
    watchedFullPct?: number;            // 0..100
    newFollowers?: number;              // in window
    ...
    retention?: {
        mostDropoffAtSec?: number;        // e.g. 2 (image shows 0:02)
    };
};
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const summaryDoc = ssnap.exists()
                    ? (ssnap.data() as AnalyticsSummary)
                    : null;
    ...
    const merged = useMemo<Required<AnalyticsSummary>>(() => {
        const fromDaily = {
            ...
            avgWatchSec:
                daily.length
                    ? Math.round(
                        daily.reduce((a, b) => a + (b.avgWatchSec || 0), 0) / daily.length
                    )
                    : 0,
            watchedFullPct:
                daily.length
                    ? Math.round(
                        daily.reduce((a, b) => a + (b.watchedFullPct || 0), 0) / daily.length
                    )
                    : 0,
            ...
            {loading ? (
                ...
                <div className="p-4 md:p-6">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                ...
                                <div className="mt-0.5 flex items-center gap-2 text-xs text-slate-500">
                                    <IoTimeOutline />
                                    <span>Posted on {postedDateStr(deed?.createdAt ?? deed?.createdAtMs)}</span>
                    ...
                    <div className="mt-6 grid gap-4 lg:grid-cols-5">
                        ...
                        <Kpi title="Average watch time" value={`${merged.avgWatchSec.toFixed(2)}s`} />
                        ...
                        <Card title="Retention rate">
                            <p className="text-sm text-slate-600">
                                ...
                                <span className="font-bold">
                                    {merged.retention.mostDropoffAtSec?.toFixed?.(0) || 0}
                                </span>
                                s. Play the video below to see when they lost interest.
                            </p>
                            <div className="mt-4 flex items-center gap-4">
                                ...
                                <div className="grow">
                                    {/* tiny progress for illustrative retention */}
