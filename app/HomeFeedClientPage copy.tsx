...
function useChannelFeed({
    ...
    async function fetchArchiveDeedsPage(page: number) {
        ...
        if (safePage === 1) {
            const firstQ = query(baseRef, ...baseConstraints, limit(PAGE_SIZE + 1));
            const firstSnap = await getDocs(firstQ);

            const pageDocs = firstSnap.docs.slice(0, PAGE_SIZE);
            const rows = pageDocs
                .map((d) => toPlayerItem(d.data(), d.id))
                .filter(Boolean) as PlayerItem[];

            return {
                items: rows,
                hasNextPage: firstSnap.docs.length > PAGE_SIZE,
            };
        }
        ...
        for (let current = 1; current < safePage; current++) {
            const stepQ = cursor
                ? query(baseRef, ...baseConstraints, startAfter(cursor), limit(PAGE_SIZE))
                : query(baseRef, ...baseConstraints, limit(PAGE_SIZE));

            const stepSnap: any = await getDocs(stepQ);
            ...
            cursor = stepSnap.docs[stepSnap.docs.length - 1];
        }

        const finalQ = cursor
            ? query(baseRef, ...baseConstraints, startAfter(cursor), limit(PAGE_SIZE + 1))
            : query(baseRef, ...baseConstraints, limit(PAGE_SIZE + 1));

        const finalSnap = await getDocs(finalQ);
        const pageDocs = finalSnap.docs.slice(0, PAGE_SIZE);
        ...
        return {
            items: rows,
            hasNextPage: finalSnap.docs.length > PAGE_SIZE,
        };
    }
    ...
    const loadInitial = useCallback(async () => {
        ...
        try {
            ...
            // SEO archive mode
            if (isArchiveMode) {
                const res = await fetchArchiveDeedsPage(archivePage);
                setItems(res.items);
            ...
            setItems(firstItems);
            setHasNextPage(allIds.length > firstIds.length);
        } finally {
            setLoading(false);
        }
    }, [tab, uid, isArchiveMode, archivePage]);
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            setHasNextPage(allIds.length > serverHydratedRef.current[tab]);
