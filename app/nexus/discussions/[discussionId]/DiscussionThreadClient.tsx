...
export default function DiscussionThreadClient({
    ...
    const [disc, setDisc] = useState<DiscussionItem | null>(initialDiscussion);
        ...
        (async () => {
            ...
            try {
                ...
                if (snap.exists()) {
                    ...
                    setDisc({
                        id: snap.id,
                        title: data?.title ?? initialDiscussion.title ?? "",
                        body: data?.body ?? initialDiscussion.body ?? "",
                        category: data?.category ?? initialDiscussion.category ?? null,
                        tags: Array.isArray(data?.tags) ? data.tags : initialDiscussion.tags ?? [],
                    });
                } else {
                    setDisc(initialDiscussion);
                }
    ...
    }, [discussionId, initialDiscussion]);
    ...
    const loadMore = useCallback(async () => {
        if (paging || !hasMore || !lastSnapRef.current) return;
        setPaging(true);
        try {
            const qRef = query(
                collection(db, "discussions", discussionId, "replies"),
                            ...
                            {isEditing ? (
                                ...
                                >
                                    ...
                                    <div className="mt-2 flex justify-end gap-2">
                                        <button
                                            onClick={cancelEdit}
                                            className="px-3 py-1.5 rounded-full border bg-white/80 hover:bg-white"
                                ...
                                <>
                                    ...
                                    <div className="mt-2 flex items-center gap-3">
                                        <button
                                            ...
                                            className="text-xs font-extrabold px-3 py-1.5 rounded-full border bg-white/70 hover:bg-white"
                                            style={{
                                                borderColor: "rgba(199,146,87,0.16)",
                                                color: EKARI.text,
                                                ...ringStyle,
                            ...
                            {isOwn ? (
                                <div className="flex items-center">
                                    <button
                                        ...
                                        className="h-9 w-9 rounded-2xl border bg-white/70 hover:bg-white grid place-items-center mr-2"
                                    ...
                                    <button
                                        ...
                                        className="h-9 w-9 rounded-2xl border bg-white/70 hover:bg-white grid place-items-center"
                                        style={{ borderColor: "rgba(199,146,87,0.16)", ...ringStyle }}
                                        title="Delete"
                                    >
        ...
        >
            <div className={cn(isDesktop ? "px-4 max-w-[1180px] mx-auto" : "px-3")}>
                <div className="h-[72px] flex items-center justify-between gap-3">
                    <button
                        onClick={goBack}
                        className="h-11 w-11 rounded-2xl border bg-white/80 backdrop-blur-xl shadow-sm grid place-items-center transition hover:bg-white"
                    ...
                    <button
                        ...
                        className="h-11 px-4 rounded-full border bg-white/80 backdrop-blur-xl shadow-sm flex items-center gap-2 transition hover:bg-white"
                        style={{ borderColor: "rgba(199,146,87,0.22)", ...ringStyle }}
                ...
                >
                    ...
                    <div className="flex items-end gap-2">
                        ...
                        <button
                            ...
                            className="h-11 w-11 rounded-2xl grid place-items-center shadow-lg"
                            style={{
                        ...
                        >
                            ...
                            <div className={clsx(isDesktop ? "px-4 py-5" : "px-3 py-4")}>
                                {hasMore ? (
                                    <button
                                        onClick={loadMore}
                                        className="rounded-2xl border px-4 py-3 text-sm font-extrabold bg-white/80 backdrop-blur-xl hover:bg-white"
                                        style={{
                                            borderColor: "rgba(199,146,87,0.18)",
                                            color: EKARI.text,
                                            ...ringStyle,
