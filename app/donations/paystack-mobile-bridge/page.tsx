...
export default function PaystackMobileBridgePage() {
  ...
  useEffect(() => {
    // If we have at least some context, attempt to open the app
    ...
    try {
      // Immediately try to open EkariHub app
      window.location.href = deepLink;
    } catch {
      // ignore – fallback UI will appear
    }

    // After a short delay, show manual "Open app" button
    ...
    <main className="min-h-screen w-full flex items-center justify-center bg-gray-100 px-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-[0_20px_40px_rgba(0,0,0,0.10)] p-5 text-center">
        ...
        <p className="mt-2 text-sm" style={{ color: EKARI.dim }}>
          We’re finishing up your donation and opening the EkariHub app so you can
          see your updated balance and support history.
        </p>
        ...
        {showFallback && (
          <div className="mt-5 space-y-2">
            ...
            <p className="text-[11px]" style={{ color: EKARI.dim }}>
              If the app doesn’t open automatically, tap the button above. Make sure
              EkariHub is installed on your phone.
            </p>
          </div>
        )}
